//! Realm setup and the fixed-tick update order.
...
impl World {
    pub fn begin_realm(&mut self, seed: u32, realm: i32) {
        // Preserve the established realm generated by a public seed. The
        // other streams derive independently so their call order can evolve.
        self.generation_rng.reseed(seed);
        self.ai_rng.reseed(stream_seed(seed, 2));
        self.combat_rng.reseed(stream_seed(seed, 3));
        self.environment_rng.reseed(stream_seed(seed, 4));
        self.cosmetic_rng.reseed(stream_seed(seed, 5));
    ...
    fn garrison_castles(&mut self) {
        for wizard in 0..=self.session.rival_count {
            ...
            for slot in 0..(GARRISON_VILLAGERS + GARRISON_SOLDIERS) {
                let angle = self.generation_rng.range(0.0, core::f32::consts::TAU);
                let radius = self.generation_rng.range(GARRISON_RING.0, GARRISON_RING.1);
    ...
    fn populate(&mut self, realm: i32) {
        ...
        for _ in 0..(WILDLIFE_BASE + realm * WILDLIFE_PER_REALM) {
            ...
            let (x, z) = World::cell_to_world(site);
            let mut kind = CreatureKind::from_index(self.generation_rng.below(4));
            if realm >= DRAGON_FROM_REALM && self.generation_rng.below(DRAGON_ODDS) == 0 {
        ...
        for _ in 0..SEEDED_ORBS {
            ...
            let (x, z) = World::cell_to_world(site);
            let amount = self.generation_rng.range(SEEDED_ORB_VALUE.0, SEEDED_ORB_VALUE.1);
    ...
    /// Presentation extraction is variable-rate and never advances gameplay.
    pub fn extract_presentation(&mut self) {
        self.build_frame();
    }

    /// Backwards-compatible combined step used by tests and old hosts.
    pub fn advance(&mut self, dt: f32) {
        self.advance_tick(dt);
        self.extract_presentation();
    }

    /// Cracked keeps smoke. It advances only the cosmetic stream, and therefore
    /// cannot affect authoritative replay state.
    fn smoke_damaged_keeps(&mut self, dt: f32) {
        ...
        for wizard in 0..=self.session.rival_count {
            ...
            if !self.cosmetic_rng.chance(PUFFS_PER_SECOND * dt) {
                continue;
            }
            let offset_x = self.cosmetic_rng.range(-PLUME_SPREAD, PLUME_SPREAD);
            let offset_z = self.cosmetic_rng.range(-PLUME_SPREAD, PLUME_SPREAD);
            let rise = self.cosmetic_rng.range(PLUME_RISE.0, PLUME_RISE.1);
            let drift = [
                self.cosmetic_rng.range(-3.0, 3.0),
                self.cosmetic_rng.range(6.0, 16.0),
                self.cosmetic_rng.range(-3.0, 3.0),
            ];
            self.spawn_particle(
                [
    ...
    fn restock_wildlife(&mut self, dt: f32) {
        ...
        let kind = if self.environment_rng.below(4) == 3 {
            CreatureKind::Griffin
        } else {
            CreatureKind::from_index(self.environment_rng.below(3))
        };
        self.spawn_creature(kind, x, z, Faction::WILD);
    }
