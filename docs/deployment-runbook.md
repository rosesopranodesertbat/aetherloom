...
The R2 bucket is private. The staging Worker intentionally has no
`MATCH_CAPACITY_API` or `BROWSER_MATCH_ORIGIN` binding until those services
exist, so allocation and WebSocket handoff return 503 rather than silently
falling back.
...
2. Run **Deploy staging** with that full commit SHA or `main`.
3. Leave `deploy_control_plane` disabled until the Worker runtime resources and
   secrets exist; enable it for control-plane staging.
4. Verify the staging web URL and, when selected, the recorded control-plane
   smoke proof.
...
reachable with the expected build. It does not create the missing match
capacity service, browser WSS origin, native QUIC fleet, session issuer, or
home-island Wasm adapter. Keep valuable progression disabled until the
end-to-end and 128-player production gates in
[the multiplayer architecture](multiplayer-architecture.md) pass.
